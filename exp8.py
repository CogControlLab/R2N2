"""
Experiment 7: Show how the replay frequency is tuned by reward rate
"""
import numpy as np
...
def validate(model,
    ...
    for k in kk:
        ...
        std = 3
        hs = (
            np.clip(np.random.normal(0, std, model.n_hA), -th, th),
            np.clip(np.random.normal(0, std, model.n_hB), -th, th),
        )
...
def reduce_replay(r, n_original):
    return int((1 - r) * n_original)


def reduce_replay_prediction_error(e, n_original):
    # use max to prevent negative value
    c = max(int(min(1 / 10 * np.log(e) + 1, 1) * n_original), 100)
    print(f"e:{e} | log e: {np.log(e)} | c: {c}")
    return c


for iii in range(7):
    exp_name = f"exp_8_FA_{iii}"
    stop_threshold = 0.85
    n_x = 11
    n_h = 64
    lr = 1e-3
    ...
    # initial replay number
    n_replays = nseq
    running_e_rate = 1
    n_replays_list = []

    for k in kk:
        n_replays = reduce_replay_prediction_error(running_e_rate, nseq)
        n_replays_list.append(n_replays)
        indices = np.random.choice(nseq, n_replays, replace=False)
        tt = 0
        for ind in tqdm(indices):
            ...
            results_list.append(
                np.all(action_prediction == sensory_seq_next[:, 0]))

            # print the average rate with some interval.
            if (tt % 100) == 0:
                kk.set_description(
                    f"Reward: {np.mean(results_list[-10:])} | Loss {np.around(np.mean(loss_list[-10:]), 2)} | {n_replays}"
                )
        running_e_rate = np.mean(loss_list[-n_replays:])
