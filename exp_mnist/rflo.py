...
exp_name = f"MNIST_RFLO-masked-{is_masked}-{mask_steps}"
...
kk = trange(n_epoch, desc="RFLO", leave=True)
loss_list_t = []
results_list_t = []
...
for e in kk:
    ...
    for i in tqdm(range(n_samples)):
        ...
        kk.set_description(
            f"RFLO | Epoch {e+1} | Train: Acc: {np.around(np.mean(results_list_t[-1000:]), 2)} | L: {np.around(np.mean(loss_list_t[-1000:]), 2)}; Validate in epoch {e}: Acc {np.around(acc_v_t, 2)}"
        )
