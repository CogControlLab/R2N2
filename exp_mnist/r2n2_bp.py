...
exp_name = f"MNIST_R2N2_BP-masked-{is_masked}-{mask_steps}"
...
for e in kk:
    ...
    for i in tqdm(range(n_samples)):
        ...
        kk.set_description(
            f"R2N2_BP | Epoch {e+1} | Train: Acc: {np.around(np.mean(results_list[-1000:]), 2)} | L: {np.around(np.mean(loss_list[-1000:]), 2)}; Validate in epoch {e}: Acc {np.around(acc_v, 2)}"
        )
